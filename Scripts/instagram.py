...
async def scrape_user_posts(username: str, page_size = 3, max_pages: Optional[int] = None):
...
# 
# 
# Partie 2 : Fonctions créée par Robin et pas par ScrapFly 
# 
# 
# 
# 
...
def load_posts_json(instagram_user, output_folder):
    """
    Charge le JSON contenant les posts d'un utilisateur et retourne les données.
    """
    file_path = Path(output_folder) / f"{instagram_user}_all-user-posts.json"
    with open(file_path, "r", encoding="utf-8") as f:
        return json.load(f)

def extract_shortcodes(posts_data):
    """
    Extrait les shortcodes de tous les posts récupérés.
    """
    return [post['shortcode'] for post in posts_data if 'shortcode' in post]

def flatten_comments(posts_data):
    """
    Fonction qui aplatit les commentaires d'une liste de posts.
    Chaque commentaire devient une ligne avec les infos du post et du commentaire.
    """
    flattened = []
    for post in posts_data:
        for comment in post.get("comments", []):
            flattened.append({
                ...
                "post_date": post.get("taken_at"),
                "comment_id": comment.get("id"),
                "comment_text": comment.get("text"),
                "comment_date": comment.get("created_at"),
                "comment_owner": comment.get("owner"),
                "comment_owner_verified": comment.get("owner_verified"),
